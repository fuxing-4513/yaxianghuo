    ...
    <div className="min-h-screen bg-cream-50">
      ...
      <section className="relative pt-24 pb-16 border-b border-gold-400/5">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center relative z-10">
          <span className="text-[11px] text-gold-400 tracking-[0.25em] uppercase">Collection</span>
          ...
          <p className="text-rich-500 max-w-lg mx-auto text-sm">缅甸矿区直供 · 每件皆为压箱之选</p>
        </div>
      </section>
      ...
      <main className="max-w-7xl mx-auto px-6 py-16">
            ...
            <button key={f} onClick={() => setFilter(f as typeof filter)}
              className={`px-6 py-2 text-xs rounded-full transition-all duration-300 tracking-wide ${
                filter === f
                  ...
                  : 'bg-cream-100/60 text-rich-400 border border-cream-200 hover:border-gold-400/20 hover:text-rich-600'
              }`}>
              {f}
            </button>
          ))}
          ...
          {filtered.map((p, i) => (
            <div key={p.id}
              ...
              <div className="aspect-square bg-gradient-to-br from-jade-950/50 via-rich-800 to-gold-900/15 flex items-center justify-center relative overflow-hidden">
                ...
                {p.tag && (
                  <span className="absolute top-3 left-3 text-[9px] bg-gold-500/80 text-rich-950 font-semibold px-2 py-0.5 rounded tracking-wider z-10">
                    {p.tag}
                  </span>
                )}
              ...
              <div className="p-5">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[10px] text-rich-300">{p.type}</span>
                </div>
                <h3 className="text-sm font-semibold text-rich-800 group-hover:text-gold-300 transition-colors font-serif">{p.name}</h3>
                <p className="text-xs text-rich-400 mt-1.5 leading-relaxed">{p.desc}</p>
                <div className="mt-4 pt-4 border-t border-cream-200/40 flex items-center justify-between">
                  <span className="text-sm font-bold text-gold-300">{p.price}</span>
                  <span className="text-[10px] text-rich-400 group-hover:text-gold-400 transition-colors">询价 →</span>
                </div>
              </div>
            </div>
          ))}
        ...
        {/* 底部说明 */}
        <div className="mt-20 text-center border-t border-gold-400/5 pt-12">
          <p className="text-xs text-rich-400 max-w-lg mx-auto leading-relaxed">
            以上为部分精选藏品展示。如需查看更多藏品或预约线下品鉴，
            <Link href="/about#contact" className="text-gold-400 hover:text-gold-300 underline underline-offset-2 mx-1">请联系我们</Link>
            预约专属服务。
          </p>
        </div>
      </main>
